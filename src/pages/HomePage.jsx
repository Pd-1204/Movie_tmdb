...
async function getMovies(pageNo) {
  ...
  function HomePage() {
     ...
     useEffect(()=>{
         getMovies(pageNo).then((res)=>{setMovies(res);
        })
        .catch((err)=>{
            alert(err);
        });
                ...
                return(
                    <div className="bg-gray-700 min-h-screen flex flex-col items-center h-full ">
                        ...
                        <div className="w-[250px] mt-5 pb-10 font-bold" >
                                    <button className="bg-white rounded-full px-4 ml-2 hover:border-black border-2 
                                    hover:font-bold" onClick={()=> {
                                        if(pageNo>1)
                                        setMovies("Loading");
                                        setPageNo(pageNo - 1);
                                    }}>
                                    ...
                                    {pageNo}
                                    <button className="bg-white rounded-full px-4 mr-2 hover:border-black border-2 
                                    hover:font-bold" onClick={()=> {
                                        if(pageNo<20)
                                        setMovies("Loading");
                                        setPageNo(pageNo + 1);
